...
:class:`~recall.schema.Event` rows and persists them through this single helper,
...
:class:`~recall.store.CapsuleStore` inline) keeps behavior identical across
...
from recall.store import DEFAULT_DB_PATH, CapsuleStore
...
def persist_events(
    ...
    Args:
        ...
        db_path: SQLite file to write to. Defaults to the shared
            :data:`~recall.store.DEFAULT_DB_PATH`; pass ``":memory:"`` in tests.
