...
The canonical home for iMessage ingest, paralleling :mod:`adaptors.spotify`. It
...
projects the records onto canonical :class:`~recall.schema.Event` rows via
:meth:`IMessageRecord.to_event`.

This keeps iMessage on the **same provenance path as every other source**: the
emitted events go through ``recall.store.CapsuleStore.add_events`` (durable
``events`` table + ``content_sha`` integrity hash), then
``recall.episodes.build_episodes`` windowing, then Hindsight ``retain`` — no
source-specific handling downstream.

The legacy module :mod:`recall.ingest` now re-exports from here, so the existing
CLI and tests keep working while this is the real implementation.

CLI::

    python -m adaptors.imessage --top-n 5 [--since YYYY-MM-DD]
"""
