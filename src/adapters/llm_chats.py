"""Canonical mapping from a parsed Claude export to :class:`~core.schema.Event` rows.
...
:class:`~core.schema.Event` (``source="claude"``), applying the project's
...
def _event_id(thread_id: str, message_uuid: str) -> str:
    ...
    matching the 16-char truncation idiom used across the adapters. The key
...
def to_chat_events(conversation: ClaudeConversation) -> list[Event]:
    """Map one conversation's messages to canonical :class:`~core.schema.Event` rows.
