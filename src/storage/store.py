...
* **active** — user-created :class:`~core.capsule.Capsule` records and their
  :class:`~core.capsule.Media` (the binaries live on disk; rows hold refs);
* **passive** — canonical :class:`~core.schema.Event` rows from sources like
...
class CapsuleStore:
    ...
    def list_events(self, source: str | None = None) -> list[Event]:
        ...
        :class:`~core.schema.Event`, so it is dropped on the way out; use
